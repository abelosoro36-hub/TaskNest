...
function Stat({ label, value, sub, color="text-[#00d4a3]", icon }: { label:string; value:string; sub:string; color?:string; icon:string }) {
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5">
      ...
      <p className={`text-2xl sm:text-3xl font-black mb-0.5 ${color}`}>{value}</p>
...
function Hdr({ title, sub }: { title:string; sub:string }) {
  return <div className="mb-6"><h2 className="text-2xl font-black text-[#0b1426]">{title}</h2><p className="text-slate-400 text-sm mt-0.5">{sub}</p></div>;
}
...
function Bar({ v, color="#00d4a3" }: { v:number; color?:string }) {
  return <div className="h-2 bg-slate-100 rounded-full overflow-hidden"><div className="h-full rounded-full" style={{ width:`${Math.min(v,100)}%`, background:color }}/></div>;
}
 
function TH({ cols }: { cols:string[] }) {
  return <thead className="bg-[#0b1426]"><tr>{cols.map(c=><th key={c} className="text-left py-3 px-4 text-slate-400 text-xs font-semibold uppercase tracking-wide whitespace-nowrap">{c}</th>)}</tr></thead>;
}
 
function Spinner() {
  return <div className="flex justify-center items-center py-16"><div className="h-8 w-8 border-4 border-[#00d4a3] border-t-transparent rounded-full animate-spin"/></div>;
}
...
function Overview() {
  ...
  const activity = [
    { u:"Amara K.",  a:"Completed AI Annotation task",     p:"+$85.00",  t:"2m ago",  av:"AK", c:"#00d4a3" },
    { u:"Diego R.",  a:"Submitted Medical Image review",   p:"+$120.00", t:"14m ago", av:"DR", c:"#3b82f6" },
    { u:"Priya M.",  a:"Passed Intermediate qualification", p:"—",        t:"32m ago", av:"PM", c:"#8b5cf6" },
    { u:"Leo F.",    a:"Withdrew $340 via PayPal",          p:"-$340.00", t:"1h ago",  av:"LF", c:"#f59e0b" },
    { u:"Sarah N.",  a:"Raised support ticket #2041",       p:"—",        t:"2h ago",  av:"SN", c:"#ef4444" },
  ];
  ...
  return (
    <div className="space-y-6">
      <Hdr title="Admin Overview" sub="Platform health at a glance — live from MongoDB." />
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <Stat label="Total Workers"   value={stats.totalWorkers?.toLocaleString()??"—"}    sub="+124 this week"    icon="👥" color="text-[#0b1426]" />
        <Stat label="Active Tasks"    value={String(stats.activeTasks??0)}                  sub="Currently live"   icon="📋" color="text-[#00d4a3]" />
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <Stat label="Open Tickets"    value={String(stats.openTickets??0)}          sub="Needs attention"  icon="🛟" color="text-red-500"    />
        <Stat label="Pending Verif."  value={String(stats.pendingVerifications??0)} sub="Awaiting review"  icon="🔒" color="text-purple-500" />
        <Stat label="Platform Acc."   value="94.2%"                                 sub="+1.3% this week"  icon="🎯" color="text-[#00d4a3]"  />
        <Stat label="Worker Tier Elite" value={String(stats.planBreakdown?.premium??0)} sub="Premium members" icon="⭐" color="text-[#f59e0b]" />
      </div>
      <div className="bg-white border border-slate-200 rounded-2xl p-6">
        ...
        <div className="space-y-4">
          {health.map(({label,v,c})=>(
            <div key={label}>
              <div className="flex justify-between mb-1.5"><span className="text-sm text-slate-600 font-medium">{label}</span><span className="text-sm font-bold text-[#0b1426]">{v}%</span></div>
              <Bar v={v} color={c}/>
            </div>
          ))}
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6">
        ...
        <div className="space-y-4">
          {activity.map(({u,a,p,t,av,c})=>(
            <div key={u+t} className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0"><p className="text-[#0b1426] font-semibold text-sm">{u}</p><p className="text-slate-400 text-xs truncate">{a}</p></div>
...
function Projects() {
  ...
  return (
    <div className="space-y-6">
      <Hdr title="Available Projects" sub="Manage all active, draft, and closing task projects." />
      ...
      <div className="flex flex-wrap gap-2 items-center">
        {["All","active","closing","draft","paused"].map(s=>(
          <button key={s} onClick={()=>setFilter(s)} className={`px-4 py-2 rounded-full text-sm font-semibold capitalize transition-all ${filter===s?"bg-[#0b1426] text-white":"bg-white border border-slate-200 text-slate-500 hover:border-slate-300"}`}>{s}</button>
        ))}
              ...
              <tbody>
                {(display as typeof staticData).map((p,i)=>(
                  <tr key={p._id} className={`border-t border-slate-100 hover:bg-[#00d4a3]/5 transition-colors ${i%2!==0?"bg-slate-50/50":""}`}>
                    ...
                    <td className="py-3 px-4"><Chip text={String(p.plan)} color={p.plan==="premium"?"yellow":"blue"}/></td>
                    ...
                    <td className="py-3 px-4"><Chip text={String(p.status)} color={p.status==="active"?"green":p.status==="closing"?"yellow":"gray"}/></td>
                    <td className="py-3 px-4"><div className="flex gap-2"><button className="text-xs text-[#00d4a3] hover:underline font-semibold">Edit</button><button className="text-xs text-red-400 hover:underline font-semibold">Pause</button></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function Training() {
  ...
  return (
    <div className="space-y-6">
      <Hdr title="Training & Qualification Center" sub="Complete modules to unlock higher-paying task categories."/>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Stat label="Your Level"  value="Inter" sub="Intermediate"       icon="⭐" color="text-[#3b82f6]"/>
      </div>
      <div className="space-y-3">
        {modules.map(m=>(
          <div key={m.id} className={`bg-white border rounded-2xl p-5 transition-all ${open===m.id?"border-[#00d4a3]/40 shadow-md":"border-slate-200"}`}>
            <div className="flex items-start justify-between gap-4">
              ...
              <button onClick={()=>setOpen(open===m.id?null:m.id)} className={`shrink-0 px-4 py-2 rounded-xl text-xs font-bold transition-all ${m.done?"bg-slate-100 text-slate-500":"bg-[#0b1426] hover:bg-[#152236] text-white"}`}>
                {m.done?"Review":"Start"}
              </button>
            </div>
            {open===m.id&&(
              <div className="mt-4 pt-4 border-t border-slate-100">
                ...
                <div className="grid grid-cols-3 gap-3 mb-4 text-center">
                  {[["Enrolled",m.enrolled.toLocaleString()],["Pass Rate",`${Math.round((m.passed/m.enrolled)*100)}%`],["Avg Score","85%"]].map(([l,v])=>(
                    <div key={l} className="bg-[#f8fafc] rounded-xl p-3"><p className="font-black text-[#0b1426] text-lg">{v}</p><p className="text-slate-400 text-xs">{l}</p></div>
                  ))}
                </div>
                <button className="w-full bg-[#00d4a3] hover:bg-[#00c494] text-[#0b1426] font-bold py-2.5 rounded-xl text-sm transition-all">{m.done?"Retake Module":"Begin Module →"}</button>
              </div>
            )}
          </div>
        ))}
...
function Earnings() {
  ...
  return (
    <div className="space-y-6">
      <Hdr title="Earnings Dashboard" sub="Track your income, payouts, and payment history."/>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Stat label="Today"      value="$342.50" sub="+12% from yesterday"  icon="💰" color="text-[#00d4a3]"/>
        <Stat label="This Week"  value="$1,140"  sub="7 tasks completed"    icon="📅" color="text-[#0b1426]"/>
        <Stat label="This Month" value="$4,280"  sub="Best month yet 🎉"    icon="📆" color="text-[#f59e0b]"/>
        <Stat label="Pending"    value="$115.00" sub="Processing in 18 hrs" icon="⏳" color="text-[#3b82f6]"/>
      </div>
      <div className="bg-white border border-slate-200 rounded-2xl p-6">
      ...
      <div className="bg-gradient-to-r from-[#0b1426] to-[#152236] border border-[#00d4a3]/20 rounded-2xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <p className="text-white font-black text-xl">Balance: <span className="text-[#00d4a3]">$227.50</span></p>
          <p className="text-slate-400 text-sm">Minimum payout $10 · Premium: 24-hr processing</p>
        </div>
        <button className="shrink-0 bg-[#00d4a3] hover:bg-[#00c494] text-[#0b1426] font-bold px-6 py-3 rounded-xl text-sm transition-all">Request Payout →</button>
      </div>
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
            ...
            <tbody>
              {history.map((h,i)=>(
                <tr key={i} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-3 px-4"><Chip text={h.status} color={h.status==="Paid"?"green":"yellow"}/></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function Quality() {
  ...
  return (
    <div className="space-y-6">
      <Hdr title="Quality & Accuracy Monitoring" sub="Track worker performance and maintain quality standards."/>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Stat label="Platform Acc." value="94.2%"  sub="+1.3% this week"     icon="🎯" color="text-[#00d4a3]"/>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <TH cols={["Rank","Worker","Tasks","Accuracy","Speed","Streak","Tier"]}/>
            <tbody>
              {workers.map((w,i)=>(
                <tr key={w.name} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2"><span className="font-bold text-[#0b1426]">{w.acc}%</span><div className="w-16"><Bar v={w.acc} color={w.acc>=90?"#00d4a3":w.acc>=80?"#f59e0b":"#ef4444"}/></div></div>
                  </td>
                  <td className="py-3 px-4 text-slate-500 text-xs">{w.speed}</td>
                  <td className="py-3 px-4 text-xs">{w.streak>0?`🔥 ${w.streak}d`:"—"}</td>
                  <td className="py-3 px-4"><Chip text={w.tier} color={tc(w.tier)}/></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function Management() {
  ...
  return (
    <div className="space-y-6">
      <Hdr title="Project Management Panel" sub="Full lifecycle control over tasks and worker submissions."/>
      <div className="bg-white border border-slate-200 rounded-2xl p-6">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <button className="flex items-center justify-center gap-2 bg-[#00d4a3] text-[#0b1426] font-bold py-3 rounded-xl text-sm hover:opacity-90 transition-all">➕ Create Project</button>
        <button className="flex items-center justify-center gap-2 bg-[#0b1426] text-white font-bold py-3 rounded-xl text-sm hover:bg-[#152236] transition-all">✅ Bulk Approve</button>
            ...
            <tbody>
              {queue.map(t=>(
                <tr key={t.id} className={`border-t border-slate-100 hover:bg-slate-50 transition-colors ${t.flag?"bg-red-50/30":""}`}>
                  ...
                  <td className="py-3 px-4"><Chip text={t.status} color={t.status==="Approved"?"green":t.status==="Flagged"||t.status==="Rejected"?"red":"yellow"}/></td>
                  <td className="py-3 px-4"><div className="flex gap-2"><button className="text-xs text-[#00d4a3] hover:underline font-semibold">Review</button>{t.flag&&<button className="text-xs text-red-400 hover:underline font-semibold">Escalate</button>}</div></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function Payments() {
  ...
  return (
    <div className="space-y-6">
      <Hdr title="Payments & Settings" sub="Manage your payout method, plan, and account preferences."/>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6">
          ...
          <div className="space-y-3">
            {[{m:"PayPal",a:"jane@example.com",p:true},{m:"Wise",a:"+254 712 345 678",p:false},{m:"Bank Transfer",a:"KE ···· 4421",p:false}].map(({m,a,p})=>(
              <div key={m} className={`flex items-center justify-between p-3 rounded-xl border ${p?"border-[#00d4a3]/40 bg-[#00d4a3]/5":"border-slate-100"}`}>
                <div><p className="text-[#0b1426] font-semibold text-sm">{m}</p><p className="text-slate-400 text-xs">{a}</p></div>
                {p?<Chip text="Primary" color="green"/>:<button className="text-xs text-[#00d4a3] font-semibold hover:underline">Set Primary</button>}
              </div>
            ))}
            <button className="w-full border border-dashed border-slate-300 hover:border-[#00d4a3] text-slate-400 hover:text-[#00d4a3] font-semibold py-2.5 rounded-xl text-sm transition-all">+ Add Method</button>
          </div>
        </div>
        <div className="bg-gradient-to-b from-[#0b1426] to-[#152236] border border-[#f59e0b]/30 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-3"><h3 className="font-black text-white">Current Plan</h3><Chip text="Premium" color="yellow"/></div>
...
function Notifications() {
  ...
  const ann=[
    ...
    {id:3,type:"Alert", title:"Maintenance Window — May 22", body:"Platform offline 02:00–04:00 UTC May 22 for scheduled maintenance.",           time:"1 day ago", pinned:false},
  ];
  const inbox=[
  ...
  return (
    <div className="space-y-6">
      <Hdr title="Notifications & Announcements" sub="Platform updates, task alerts, and your message inbox."/>
      <div className="space-y-3">
        {ann.filter(a=>!dismissed.includes(a.id)).map(a=>(
          <div key={a.id} className={`bg-white border rounded-2xl p-5 relative ${a.pinned?"border-[#00d4a3]/40":"border-slate-200"}`}>
            {a.pinned&&<span className="absolute top-3 right-12 text-[#00d4a3] text-xs font-bold">📌 Pinned</span>}
            <button onClick={()=>setDismissed(p=>[...p,a.id])} className="absolute top-3 right-4 text-slate-300 hover:text-slate-500 text-xl leading-none">×</button>
        ...
        <div className="divide-y divide-slate-100">
          {inbox.map(({from,subject,time,read},i)=>(
            <div key={i} className={`flex items-start gap-4 px-6 py-4 hover:bg-slate-50 cursor-pointer transition-colors ${!read?"bg-[#00d4a3]/5":""}`}>
              ...
              <div className="flex-1 min-w-0"><p className={`text-sm ${!read?"font-bold text-[#0b1426]":"font-medium text-slate-600"}`}>{from}</p><p className="text-slate-500 text-sm truncate">{subject}</p></div>
...
function Analytics() {
  ...
  return (
    <div className="space-y-6">
      <Hdr title="Community & Performance Analytics" sub="Platform-wide trends, worker growth, and task performance."/>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6">
        ...
        <div className="flex items-end gap-3 h-36">
          {monthly.map(({m,v})=>(
            <div key={m} className="flex-1 flex flex-col items-center gap-1.5"><p className="text-xs font-bold text-[#0b1426]">${v}k</p><div className="w-full rounded-t-lg" style={{height:`${(v/92)*120}px`,background:m==="May"?"#00d4a3":"#0b1426"}}/><p className="text-xs text-slate-400">{m}</p></div>
          ))}
        </div>
      </div>
    </div>
...
function Security() {
  ...
  return (
    <div className="space-y-6">
      <Hdr title="Account Verification & Security" sub="Manage identity verification, 2FA, and account security."/>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between p-4 bg-[#f8fafc] rounded-xl border border-slate-100">
            <div><p className="font-semibold text-[#0b1426] text-sm">Two-Factor Authentication (2FA)</p><p className="text-slate-400 text-xs">Protect with an authenticator app</p></div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <TH cols={["Name","Email","Country","Actions"]}/>
            <tbody>
              {pending.map(p=>(
                <tr key={p.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                  <td className="py-3 px-4 font-semibold text-[#0b1426]">{p.name}</td>
                  <td className="py-3 px-4 text-slate-500 text-xs">{p.email}</td>
                  <td className="py-3 px-4 text-slate-500 text-xs">{p.country}</td>
                  <td className="py-3 px-4"><div className="flex gap-2"><button className="text-xs font-bold text-[#00d4a3] hover:underline">Approve</button><button className="text-xs font-bold text-red-400 hover:underline">Reject</button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function Support() {
  ...
  return (
    <div className="space-y-6">
      <Hdr title="Support Center" sub="Manage all worker support tickets and escalations."/>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <TH cols={["User","Subject","Priority","Status","Agent","Action"]}/>
            <tbody>
              {tickets.map(t=>(
                <tr key={t.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                  <td className="py-3 px-4 font-semibold text-[#0b1426] whitespace-nowrap">{t.name}</td>
                  <td className="py-3 px-4 text-slate-500 text-sm max-w-xs truncate">{t.subject}</td>
                  <td className="py-3 px-4"><Chip text={t.priority} color={pc(t.priority)}/></td>
                  <td className="py-3 px-4"><Chip text={t.status}   color={sc(t.status)}  /></td>
...
function Review() {
  ...
  return (
    <div className="space-y-6">
      <Hdr title="Task Review System" sub="QA queue — approve, flag, or reject worker submissions."/>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">
            {queue.map(q=>(
              <div key={q.id} onClick={()=>{setSel(q.id);setNote("");}} className={`px-5 py-4 cursor-pointer transition-all ${sel===q.id?"bg-[#00d4a3]/8 border-l-4 border-[#00d4a3]":"hover:bg-slate-50"}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1"><span className="font-mono text-xs text-slate-400">{q.taskId}</span><Chip text={q.risk==="None"?"Clean":q.risk+" Risk"} color={rc(q.risk)}/></div>
                    <p className="font-semibold text-[#0b1426] text-sm">{q.worker}</p>
                    <p className="text-slate-400 text-xs">{q.project} · {q.time}</p>
                  </div>
                  <div className="text-right shrink-0"><p className={`font-black text-lg ${q.acc>=85?"text-[#00d4a3]":q.acc>=75?"text-[#f59e0b]":"text-red-500"}`}>{q.acc}%</p><p className="text-slate-400 text-xs">accuracy</p></div>
                </div>
                {q.flags.length>0&&<div className="mt-2 flex flex-wrap gap-1">{q.flags.map(f=><span key={f} className="text-xs bg-red-50 text-red-500 border border-red-100 px-2 py-0.5 rounded-full">{f}</span>)}</div>}
                {dec[q.id]&&<p className={`mt-1.5 text-xs font-bold ${dec[q.id]==="Approved"?"text-[#00d4a3]":dec[q.id]==="Flagged"?"text-[#f59e0b]":"text-red-500"}`}>→ {dec[q.id]}</p>}
              </div>
            ))}
          </div>
        </div>
        <div className="bg-white border border-slate-200 rounded-2xl p-6">
          {item?(
            <div className="space-y-5">
              <div>
                <div className="flex items-center gap-2 mb-2"><span className="font-mono text-xs text-slate-400">{item.taskId}</span><Chip text={item.risk==="None"?"Clean":item.risk+" Risk"} color={rc(item.risk)}/></div>
              ...
              <div className="bg-[#f8fafc] rounded-2xl p-4">
                ...
                <Bar v={item.acc} color={item.acc>=85?"#00d4a3":item.acc>=75?"#f59e0b":"#ef4444"}/>
                <p className="text-slate-400 text-xs mt-2">{item.acc>=85?"Meets quality standard ✓":item.acc>=75?"Below standard — review flags":"Fails quality standard"}</p>
              </div>
              {item.flags.length>0&&(
                <div>
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Quality Flags</p>
                  <div className="space-y-1.5">{item.flags.map(f=><div key={f} className="flex items-center gap-2 bg-red-50 border border-red-100 rounded-lg px-3 py-2"><span className="text-red-400 text-xs">⚠️</span><span className="text-red-600 text-xs font-medium">{f}</span></div>)}</div>
                </div>
              )}
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Reviewer Note</p>
                <textarea rows={3} value={note} onChange={e=>setNote(e.target.value)} placeholder="Add feedback for the worker (optional)…" className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-[#0b1426] placeholder-slate-400 focus:outline-none focus:border-[#00d4a3] focus:ring-2 focus:ring-[#00d4a3]/20 transition-all resize-none"/>
              </div>
              <div className="grid grid-cols-3 gap-3">
                {(["Approved","Flagged","Rejected"] as const).map(d=>(
                  <button key={d} onClick={()=>setDec(p=>({...p,[item.id]:d}))}
                    className={`py-3 rounded-xl text-sm font-bold transition-all ${
                      dec[item.id]===d?d==="Approved"?"bg-[#00d4a3] text-[#0b1426]":d==="Flagged"?"bg-[#f59e0b] text-[#0b1426]":"bg-red-500 text-white"
    ...
    <div className="min-h-screen bg-[#f1f5f9] flex">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-64 bg-[#0b1426] flex flex-col transition-transform duration-300 ${sidebarOpen?"translate-x-0":"-translate-x-full lg:translate-x-0"}`}>
        ...
        <div className="p-4 border-t border-white/10 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0"><p className="text-white text-xs font-semibold truncate">Admin User</p><p className="text-slate-500 text-xs truncate">admin@tasknest.io</p></div>
